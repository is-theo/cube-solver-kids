...
본 프로젝트는 `vitest`를 사용하여 주요 비즈니스 로직의 정확성을 검증합니다.
...
### 테스트 범위 (Scope)
*   **단위 테스트 (Automated)**: 큐브 솔버 알고리즘, 색상 공간 변환(Lab/Delta E 2000), 좌표 계산 유틸리티 등 핵심 로직을 검증합니다.
*   **권장 수동 검증**: 브라우저 API 및 기기 하드웨어 의존성이 높은 다음 기능은 실제 기기 환경에서의 확인이 필요합니다.
    - MediaPipe/OpenCV 기반 실시간 분석 루프의 성능 및 안정성
    - 다양한 해상도 및 화면 비율에서의 그리드 정렬
    - 카메라 권한 획득 및 스트림 제어 (전/후면 전환 등)

> 💡 **상세 가이드**: 실제 기기 테스트 시 발생하는 이슈는 [문제 해결 가이드](./docs/TROUBLESHOOTING.md)를 참고하세요.
...
2. 흰 → 빨강 → 초록 → 노랑 → 주황 → 파랑 순으로 면 캡처
   ...
   - **수동 조정**: 자동 정렬이 잘 안 될 경우, 화면의 4개 코너를 직접 드래그하여 맞출 수 있습니다.
