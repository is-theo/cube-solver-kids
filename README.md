...
Vitest를 사용하여 핵심 비즈니스 로직(색상 분류 알고리즘, 큐브 상태 검증 등)을 테스트합니다.

```bash
npm test
```

> 💡 **참고**: 현재 유닛 테스트는 순수 로직에 집중되어 있습니다. 카메라 스트림(`getUserMedia`), 브라우저 권한, 3D 렌더링 등 환경 의존적인 부분은 테스트 범위에서 제외되어 있으며, 수동 브라우저 테스트가 필요합니다.
