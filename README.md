...
### 테스트 범위 (Scope)
*   **핵심 알고리즘**: RGB → Lab 변환, 델타 E 기반 색상 분류, 큐브 상태 검증 및 Facelet 변환 (단위 테스트 30개 통과).
*   **브라우저 로직**: `extract9Cells` 등 기하학적 보간 로직 검증.
*   **수동 검증 대상**:
    *   **카메라 하드웨어 제어**: 전면/후면 카메라 전환, 노출/화이트밸런스 잠금 기능.
    *   **UI/UX**: 실제 장치에서의 3D 렌더링 및 카메라 오버레이 반응성.
    *   카메라 전환 기능은 다양한 Facing Mode 설정을 지원하는 장치에서 실제 동작을 확인해야 합니다.
